#!/usr/bin/env python3
"""
Publish agentic extraction results to Google Sheets with an explanation tab.

Creates two tabs:
  1. Results tab — clear-and-rewrite with v2 extraction data
  2. Explanation tab — plain-language overview for non-technical readers

Usage:
    python scripts/publish_to_sheets.py
    python scripts/publish_to_sheets.py --dry-run
    python scripts/publish_to_sheets.py --results-tab "Custom Name" --no-explanation
"""
...
from src.config import OUTPUT_DIR, OUTPUT_SHEET_ID
...
DEFAULT_CSV = OUTPUT_DIR / "agentic_v2.csv"
...
def publish_results_tab(gc, rows: list[list[str]], tab_name: str) -> int:
    """Clear and write results to a Google Sheets tab."""
    import gspread
    ...
    try:
        ...
        worksheet.clear()
        logger.info(f"Cleared existing worksheet: {tab_name}")
    except gspread.WorksheetNotFound:
        worksheet = sheet.add_worksheet(
            title=tab_name, rows=len(rows) + 10, cols=len(SHEET_HEADERS) + 2
        )
    ...
    # Format: freeze header, bold header, auto-resize
    ...
    logger.info(f"Published {len(rows)} rows to '{tab_name}'")
    return len(rows)
...
def build_explanation_rows() -> list[list[str]]:
    ...
    lines = [
        ...
        "",
        "一、本表說明",
        "本資料表包含 3 家企業 2024 年度 ESG 永續報告書的結構化萃取結果，共 266 筆欄位資料。",
        ...
        "  步驟 2：與 Google Gemini API 萃取結果交叉比對，找出差異",
        "  步驟 3：針對 80 筆差異，逐筆回到 PDF 原文人工核對",
        "  步驟 4：根據核對結果修正萃取指令，重新萃取並再次驗證",
        "  經兩輪迭代修正後，80 筆驗證欄位全數正確（準確率 100%）",
        "",
        "六、頁碼說明",
...
def publish_explanation_tab(gc, tab_name: str) -> int:
    """Write explanation tab to Google Sheets."""
    import gspread
    ...
    try:
        ...
        worksheet = sheet.add_worksheet(title=tab_name, rows=100, cols=3)
        logger.info(f"Created new worksheet: {tab_name}")

    rows = build_explanation_rows()
    worksheet.append_rows(rows)

    # Format title (row 1): bold, larger font
    worksheet.format("A1", {"textFormat": {"bold": True, "fontSize": 14}})

    # Format section headers (一、二、三…)
    for i, row in enumerate(rows, start=1):
        if row and row[0] and len(row[0]) > 1 and row[0][0] in "一二三四五六七八九十":
            worksheet.format(f"A{i}", {"textFormat": {"bold": True, "fontSize": 11}})

    # Set column A width and enable text wrap
    sheet.batch_update({
        "requests": [{
    ...
    worksheet.format(f"A1:A{len(rows)}", {"wrapStrategy": "WRAP"})

    logger.info(f"Published explanation tab with {len(rows)} rows")
    return len(rows)


def main():
    parser = ArgumentParser(
        description="Publish agentic extraction results to Google Sheets"
    )
    parser.add_argument(
    ...
    setup_logging(session_name="publish_to_sheets")
    logger = get_logger()
    ...
    if args.dry_run:
        ...
        print(f"[DRY RUN] Spreadsheet: https://docs.google.com/spreadsheets/d/{OUTPUT_SHEET_ID}")
    ...
    if gc is None:
        logger.error(
            ...
            "Ensure credentials.json exists or GOOGLE_APPLICATION_CREDENTIALS is set."
        )
        sys.exit(1)

    results_count = publish_results_tab(gc, rows, args.results_tab)

    explanation_count = 0
    if not args.no_explanation:
        explanation_count = publish_explanation_tab(gc, args.explanation_tab)

    print(f"\n{'='*60}")
    print("Publish to Sheets — Summary")
    print(f"{'='*60}")
    ...
    print(f"Spreadsheet:     https://docs.google.com/spreadsheets/d/{OUTPUT_SHEET_ID}")
    print(f"{'='*60}")
