#!/usr/bin/env python3
"""
Run extraction on newly downloaded PDFs.

Processes PDFs from new_pdfs/ and writes results to a new Google Sheets tab.

Usage:
    python scripts/run_new_extraction.py
    python scripts/run_new_extraction.py --dry-run
"""
...
# Sheet name for new extraction results
SHEET_NAME = "新公司驗證 26-02-05（prompt ver. 5）"

# PDFs directory - using finance PDFs (10 financial institutions)
NEW_PDFS_DIR = Path(__file__).parent.parent / "finance_pdfs"

# Company info mapping (code -> info) - 10 Financial Institutions
NEW_COMPANIES = {
...
def parse_pdf_filename(filename: str) -> dict:
    ...
    Example: 2881_富邦金_2024_永續報告書.pdf
    """
    parts = filename.replace('.pdf', '').split('_')

    # Standard format: {code}_{name}_{year}_永續報告書.pdf
    if len(parts) >= 3:
        code = parts[0]
        name = parts[1]
        year = parts[2]
        company_info = NEW_COMPANIES.get(code, {"name": name, "industry": "金融業"})
        return {
            'company_code': code,
...
def create_or_get_worksheet(gc: gspread.Client, sheet_name: str):
    ...
    try:
        ...
        # Add instruction row
        worksheet.update_cell(1, 1, "說明：新公司 prompt ver. 5 提取結果")
...
def run_extraction(dry_run: bool = False) -> None:
    """Run extraction on all new PDFs."""
    logger = get_logger()

    # Get all PDF files
    pdf_files = sorted(NEW_PDFS_DIR.glob("*.pdf"))

    if not pdf_files:
        logger.error(f"No PDF files found in {NEW_PDFS_DIR}")
        return

    print(f"\n{'='*60}")
    print("New Companies Extraction Plan (Ver. 5)")
    print(f"{'='*60}")
    print(f"PDF Directory: {NEW_PDFS_DIR}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run extraction on new company PDFs"
    )
    parser.add_argument(
        "--dry-run", "-n",
        action="store_true",
    ...
    setup_logging(session_name="new_extraction")
